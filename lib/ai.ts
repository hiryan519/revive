...
import type { TaskCitation, TaskResult } from "@/lib/types";
...
export async function runTaskWithModel(query: string, contexts: ContextChunk[]): Promise<TaskResult> {
...
4. 当参考内容中存在观点冲突时，不要回避冲突，处理方式如下：
   - 直接给出你认为更适合用户当前任务的方案
   - 在引用部分注明“关于[X]，不同来源存在差异：来源A建议[X1]，来源B建议[X2]，本次结果采信来源A，原因是其描述更系统完整/更贴近用户任务场景”
